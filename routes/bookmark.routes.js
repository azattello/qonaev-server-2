    ...
    try {
        ...
        for (const bookmark of bookmarks) {
            ...
            if (!track) {
                ...
                bookmark.price = user.personalRate ? (parseFloat(track.weight) * parseFloat(user.personalRate)).toFixed(2) : track.price || 'Неизвестно'; // Добавляем цену с учетом персонального тарифа
                bookmark.place = track.place || '-'; // Добавляем место
                bookmark.weight = track.weight || 'Неизвестно'; // Добавляем вес
                bookmark.currency = track.currency || 'Неизвестно'; // Добавляем валюту

                // Записываем ID трека, текущий статус, цену, вес и валюту в базу данных
                await User.updateOne(
                    { _id: userId, 'bookmarks._id': bookmark._id },
                    {
                        $set: {
                            ...
                            'bookmarks.$.price': bookmark.price,
                            'bookmarks.$.place': bookmark.place,
                            'bookmarks.$.weight': bookmark.weight,
                            'bookmarks.$.currency': bookmark.currency
                        }
                    }
                );
